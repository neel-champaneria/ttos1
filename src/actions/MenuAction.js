import {
    fetchMenuService, fetchAppConfigService, uploadCategoryImageService, saveItemService, commentService,
    saveCategoryService, saveModifierService, saveModifierGroupService
} from '../services/MenuService';
import {
    APP_IS_LOADING, FETCH_MENU_SUCCESS, APP_LOAD_CONFIGURATION_SUCCESS, UPLOAD_ITEM_IMAGE_SUCCESS,
    UPLOAD_ITEM_IMAGE_FAIL, UPLOAD_CATEGORY_IMAGE_FAIL, UPLOAD_CATEGORY_IMAGE_SUCCESS, FETCH_MENU_FAIL,
    LOAD_RES_PROFILE_SUCCESS, LIST_DELIVERY_ZONE_SUCCESS_FOR_OPEN_USER, LIST_ORDER_AHEAD_DAYS_SUCCESS, FETCH_MENU_FOR_ITEM_SETTING_SUCCESS
} from '../constants';
...
export const commentAction = (item, comment, commentDate) => {
    return async (dispatch, getState) => {
        ...
        const resp = await commentService({itemId: id, comment, commentDate, userInfo}, user);
        if (resp.status) {
...
export const saveModifierAction = (data) => {
    return async (dispatch, getState) => {
        ...
        if (resp?.status) {
            ...
            const foundModGroup = foundItem.modifierGroupItems.find(itm => itm.modifierGroup.id == data.modGroupId).modifierGroup;
            const foundModifier = foundModGroup.modifiers.find(itm => itm.id == data.modifierId);
